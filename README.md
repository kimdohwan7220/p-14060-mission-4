...
#### 🔍 목록
- 전체 명언을 조회, 검색할 수 있다.
- `keywordType`과 `keyword`를 이용해 `명언 내용(content)` 또는 `작가(author)` 기준으로 검색한다.
- 검색 결과는 `부분 일치`로 필터링되며, 최신 등록 순(ID 내림차순)으로 출력한다.
...
#### 💾 파일 저장 (영속성)
- 등록, 수정, 삭제된 명언은 파일 형태로 저장한다.
  - 저장 경로 : `db/wiseSaying/`
  - 각 명언은 `{id}.json` 형태로 저장한다.
  - 마지막 명언 ID는 `lastId.txt`에 저장되어 다음 ID 자동 증가에 사용한다.
  - `빌드` 명령 실행 시 전체 목록이 `data.json` 파일로 생성/갱신한다.
  - [x] 프로그램을 다시 실행해도 데이터가 유지한다.